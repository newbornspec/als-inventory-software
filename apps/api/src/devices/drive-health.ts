// How a drive's health (contract C5, profile.storage[i].health) is WORDED -
// on the asset page, in the Audit workspace and in the xlsx reports. One
// formatter so the same drive cannot read "94% Good" in one place and
// something else in the next.
//
// A deliberate COPY lives in apps/web/lib/drive-health.ts: the web app cannot
// import API code. apps/api/src/devices/drive-health.spec.ts runs every case
// through BOTH files, so they cannot drift silently. Change both together, and
// keep this file dependency-free (no imports) so the copy stays byte-identical
// below this header.
//
