...
// for a human — on the asset page, and now in the xlsx reports the same way each
// drive's health is. One formatter so the same device cannot read "Passed" in
// one place and "Failed" in the next.
//
// A deliberate COPY lives in apps/web/lib/hardware-test.ts: the web app cannot
// import API code, and the asset-page card must word a result exactly as the
// batch and pallet reports do. apps/api/src/devices/hardware-test.spec.ts runs
// every case through BOTH files, so the two cannot drift silently. Change both
// together, and keep this file dependency-free (no imports) so the copy stays
// byte-identical to the web file below this header.
//
