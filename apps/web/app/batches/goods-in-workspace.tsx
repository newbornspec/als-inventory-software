...
// The subset of the captured hardware profile the specs panel renders. Every
// field optional — a wiped, live-booted machine yields a different set each
// time, and hand-entered devices may carry almost nothing.
interface Profile {
  ...
  battery?: { health?: string; designCapacity?: string; fullChargeCapacity?: string; cycleCount?: number };
}
...
// Flatten the captured profile into the mockup's component table. Only rows
// with at least a model or a size are emitted — an empty category line says
// nothing worth a row.
function specRows(p: Profile | null | undefined): SpecRow[] {
  if (!p) return [];
  const rows: SpecRow[] = [];
  ...
  if (
    // Guard on every field the row can render — a capture holding only
    // designCapacity + cycleCount is a legal shape and still deserves a row.
    p.battery?.health ||
...
export function GoodsInWorkspace({
  ...
  const rows = specRows((detail.asset?.hardwareProfile as Profile | null) ?? null);
