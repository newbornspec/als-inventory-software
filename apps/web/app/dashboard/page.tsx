...
import { Bar, Empty, OkBadge, RowLink, Section, SeverityBadge, TableScroll, Tile } from './components';
...
const TH = 'px-4 py-2 text-left text-xs font-medium uppercase tracking-wide text-neutral-600';
  ...
  return (
    <>
      ...
      <main id="main-content" tabIndex={-1} className="min-h-screen bg-white text-neutral-950 px-4 py-6 sm:px-8 sm:py-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-neutral-600">
              ...
              {user?.role ? ` · ${formatLabel(user.role)}` : ''} · Inventory health:{' '}
              {/* The word carries the state; the badge colour only repeats it. */}
              <strong className="font-semibold">{healthWord}</strong>
            </p>
          </div>
        ...
        <form action="/assets" method="get" className="mt-5 flex max-w-md gap-2">
          ...
          <input
            ...
            className="field-underline min-w-0 flex-1 px-3 py-2 text-sm"
          />
          <button
            type="submit"
            className="shrink-0 rounded-lg bg-[#1a6ef5] px-4 py-2 text-sm font-medium text-white hover:bg-blue-600"
          >
            Search
          </button>
        </form>
        ...
        <Section
          ...
          action={
            canManage ? (
              <Link href="/reports" className="text-sm font-medium text-blue-800 hover:underline">
                Full reports
              </Link>
            ) : undefined
          }
        ...
        <div className="grid min-w-0 gap-8 lg:grid-cols-2">
          ...
          <Section
            ...
            action={
              canManage ? (
                <Link href="/activity" className="text-sm font-medium text-blue-800 hover:underline">
                  View all activity
                </Link>
              ) : undefined
            }
