...
#   - in the stock cloud-init package every unit also carries
#     ConditionPathExists=!/etc/cloud/cloud-init.disabled - but NOT on the
#     station: the 24.04.2 desktop image replaces cloud-init.service with one
#     "Automatically generated by installer build process" that has no
#     Condition lines. That costs nothing, because the generator is the only
#     thing that pulls cloud-init.target in, and nothing on the image
#     Requires= a cloud-init unit (checked against the real ISO's layers) -
#     but do not count on the condition as a second guard.
# The gain is more than cloud-init's own 7 s: gdm.service is
# After=cloud-config.service, which is After=network-online.target, and
# cloud-init-local is Before=sysinit.target - so gdm was also waiting out
# NetworkManager-wait-online, and everything waited out cloud-init-local.
